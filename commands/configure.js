...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand =>
      subcommand
        .setName('ids')
        .setDescription('List of configuration IDs')),
  async execute(interaction) {
    if (!interaction.member.permissions.has(PermissionFlagsBits.Administrator)) {
      console.log(`[Command_Handling] ${interaction.user.tag} denied to use command: /configure`)
    ...
    if (sub === 'ids') {
      const id = new EmbedBuilder()
        ...
        .setColor('#73a6ff')
        .addFields(
          { name: 'c01', value: `Confession_Send // Set To : **${conf}**`, inline: false },
          { name: 'm01', value: `Moderator_Log // Set To : **${modlog}**`, inline: false },
        );
