  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      ...
      async execute(interaction) {
        const y = interaction.options.getString('aotd_id');
        const h = '1091243606142111874';
        const t = interaction.client.channels.cache.get(h);
        if (!t) {
          return interaction.reply({ content: `Unable to find aotd channel.`, ephemeral: true });
        }
        ...
        const confessEmbed = new EmbedBuilder()
          ...
          .addFields({name: 'Answered By', value: `**${o}**`})
          .setFooter({text: `AOTD Submitter: ${interaction.user.tag}`})
        ...
        try {
          ...
          console.error(error);
          interaction.followUp({ content: `I was unable to send a message. Please try again later.`, ephemeral: true });
        }
      }
    }
}      